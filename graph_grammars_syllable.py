...
def all_pair_correlations(sentence_length, all_sentences):
	...
	all_letter_pairs=list(itertools.combinations(alphabet, 2))
	letter_pair_positional_cors={} #(0,1): [cor_ij for letters 0 and 1 for all i and j position]
	positionals=list(itertools.combinations(range(n), 2))
	...
	for pair in all_letter_pairs:
			...
			for position_pair in positionals:	
				...
				for j in range(0,SENTENCE_N): #there has to be a way to map instead of iterating. also make this a separate function for clarity FIXME
					if (len(all_sentences[j])<n):
                                            tttt=n-len(all_sentences[j])
                                            addstring='i'*tttt
                                            all_sentences[j]=all_sentences[j]+addstring
...
def map_syllablles(word, dic):
	#word is an array of digits
	#dic is a dictionary digit -> syllable
	full_word=''
	for i in word:
		#print(str(i))
		full_word=full_word+dic[i]
	return full_word
...
for l in range(0,1):#20
	...
	for n in range(8,9):#100
		...
		for i in range(0,f):
		    ...
		    copy_strings.append(map_syllablles(copy_string,syllable_dict))
		    mirror_strings.append(map_syllablles(mirror_string,syllable_dict))
		    random_strings.append(map_syllablles(r_string,syllable_dict))	
		    type0_strings.append(map_syllablles(type0_string,syllable_dict))	
		...
		copy_cor_mean=np.zeros((n*6,n*6))	
		...
		copy_total_cor=np.zeros((n*6,n*6))	
		for val in copy_strings_cors.values():
		...
		mirror_cor_mean=np.zeros((n*6,n*6))	
		...
		random_total_cor=np.zeros((n*6,n*6))	
		for val in random_strings_cors.values():
			random_total_cor = random_total_cor + val - random_cor_abs_mean


		type0_cor_mean=np.zeros((n*6,n*6))	
		...
		type0_total_cor=np.zeros((n*6,n*6))	
		for val in type0_strings_cors.values():
